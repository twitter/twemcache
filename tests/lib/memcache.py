...
class Client(local):
    ...
    def set_stats(self, arg):
        '''set stats options, such as detail on/off.
           for detail dump, use get_stats("detail dump") instead
        '''
    ...
    def get_slabs(self):
        ...
        for s in self.servers:
            ...
            while 1:
                line = readline()
                if not line or line.strip() == 'END': break
                line = line.split(' ', 1)[1] #0 = STAT, #1 = <cid>:<key> <val>
                cid, metric = line.split(':', 1) #0 = <cid>, #1 = <key> <val>
                cid = int(cid)
                key, value = metric.split(' ', 1)
                if cid not in serverData:
                    serverData[cid] = {}
                serverData[cid][key] = value
            data.append(( name, serverData ))
        return data

    def flush_all(self, time=None):
        'Expire all data currently in the memcache servers.'
        for s in self.servers:
            if not s.connect(): continue
            if time:
                s.send_cmd('flush_all %d' % time)
            else:
    ...
    def gets_multi(self, keys, key_prefix=''):
        '''
        Retrieves multiple keys from the memcache doing just one query with cas id,
        used in conjunction with 'cas'.
        See also L{set_multi}.
        ...
        for server in server_keys.iterkeys():
            try:
                line = server.readline()
                while line and line != 'END':
                    rkey, flags, rlen, cas_id, = self._expect_cas_value(server, line)
                    if rkey is not None:
                        self.cas_ids[rkey] = cas_id
                        val = self._recv_value(server, flags, rlen)
                        retvals[prefixed_to_orig_key[rkey]] = val   # un-prefix returned key.
